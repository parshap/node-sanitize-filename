...
function sanitize(input, replacement, invalids) {
  ...
  if (typeof replacement === 'string' && invalids.indexOf(replacement) !== -1) {
    throw new Error(`The replacement string can't be part of options.invalid or contain substrings which are part of options.invalid!`);
  }
  var sanitized = input;
  while (invalids.some(invalid => sanitized.includes(invalid))) {
    ...
    if (counter > 1000) {
      throw new Error(`Invalid replacement function. Make sure that replacements generated by your function do not contain any of the strings specified in options.invalid!`);
    } else {
      counter++;
    }
  }
